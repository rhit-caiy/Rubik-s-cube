...
htm={}
qtm={}
cubelist=[[cblock.copy(),cblockd.copy(),1,-1,True]]
newcubelist=[]
key=cdict[tuple(cblock)]*2187+cpddict[tuple(cblockd)]
htm[key]=1
qtm[key]=1
tstart=time.time()
print("{:<6}{:<12}{:<12}{}".format("step","htm","qtm","time"))
print("{:<6}{:<12}{:<12}{:f}".format(0,1,1,0))
for step in range(1,17):
    t1=time.time()
    for cube in cubelist:
        c,cd,step,f1,isqtm=cube
        for f in range(3):
            for t in range(1,4):
                nc=c.copy()
                ncd=cd.copy()
                ...
                newstep=step*9+f*3+t-1
                isqtm2=isqtm and t!=2 and key not in qtm
                if key not in htm:
                    htm[key]=newstep
                    newcubelist.append([nc,ncd,newstep,f,isqtm2])
                elif key not in qtm and isqtm and t!=2:
                    newcubelist.append([nc,ncd,newstep,f,isqtm2])
                if isqtm2:
                    qtm[key]=newstep
    print("{:<6}{:<12}{:<12}{:f}".format(step,len(htm),len(qtm),time.time()-t1))
    if newcubelist==[]:
        break
