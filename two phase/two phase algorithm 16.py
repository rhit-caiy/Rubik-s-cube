...
def solve(c,co,eo,e1,e2,e3,threadid,lock,htm,qtm,minmove,verifiednum,stepshouldbelow,phase1maxstep,dict2step,dict1,dict2,cdict,codict,ep4dict,eodict,cr,cor,ep4r,eor,ccn,ccon,cen1,cen2,cen3,ceon,cr0,cor0,eor0,ep4r0,cr1,ep4r1):
                            ...
                            if key2 in dict2:
                                ...
                                if solution<minmove.value:
                                    ...
                                    if qtmvalue<qtm.value:
                                        qtm.value=qtmvalue
                                    print("{:<8}{:<18}{:<6}{:<24}{:<14f}1     {:<36}{}\n".format(threadid,f"{htm.value} = {step} + {htm.value-step-p2} + {p2}",qtmvalue,str(solutionnum)+"/"+str(totalnum),time.time()-tstart,solution,numstr),end="")
                                    lock.release()
                            if int(log(m1,18))+dict2step<=htm.value or key2 in dict2 and int(log(solution,18))-1<=htm.value:
                                ep4rf01=ep4r1[f0]
...
if __name__=="__main__":
    ...
    def getdicts():
        global cdict,codict,ep4dict,eodict,cr,cor,ep4r,eor,ccn,ccon,cen1,cen2,cen3,ceon,cr0,cor0,eor0,ep4r0,cr1,ep4r1
    ...
    manager=Manager()
    #dict1=manager.dict(dict1)
    #dict2=manager.dict(dict2)
    htms,qtms=[],[]
    verifiednum=manager.list()
    lock=Lock()
    ...
    for i in range(cubenumber):
        ...
        #threadsolutions=manager.Array("i",[0]*threadn)
        '''
        po=Pool(threadn)
        t1=time.time()
        for p in range(threadn):
            po.apply_async(solve,args=(*unsolvedcubes[p],p,lock,htm,qtm,minmove,verifiednum,stepshouldbelow,phase1maxstep,dict2step,dict1,dict2,cdict,codict,ep4dict,eodict,cr,cor,ep4r,eor,ccn,ccon,cen1,cen2,cen3,ceon,cr0,cor0,eor0,ep4r0,cr1,ep4r1))
        po.close()
        po.join()
        t2=time.time()
        '''
        
        processes=[Process(target=solve,args=(*unsolvedcubes[p],p,lock,htm,qtm,minmove,verifiednum,stepshouldbelow,phase1maxstep,dict2step,dict1,dict2,cdict,codict,ep4dict,eodict,cr,cor,ep4r,eor,ccn,ccon,cen1,cen2,cen3,ceon,cr0,cor0,eor0,ep4r0,cr1,ep4r1)) for p in range(threadn)]
        t1=time.time()
        print("start")
        ...
        vn=[i for i in verifiednum][-threadn:]
        print("completed phase one number",vn,sum(vn))
        ...
        if htm.value>=stepshouldbelow:
            ...
            for j in range(threadn):
                ...
                if minmove==threadsolutions[j]:
                    ...
                    bestthread=j
            '''
            bestrotation=decodevalue(minmove.value)
            htms.append(htm.value)
            qtms.append(qtm.value)
            #print("\nbest solution in thread",bestthread)
            print("min htm",htm.value,", qtm",qtm.value,"solution",minmove.value,bestrotation,rotatenumbertostring(bestrotation))
    ...
    vn=[i for i in verifiednum]
    print("average phase 1 completed number per thread",sum(vn)/len(vn),"max",max(vn),"min",min(vn))
    print("search depth",phase1maxstep,"+",dict1step,"+",dict2step,"=",stepshouldbelow-1)
    print("htm",htms)
    print("qtm",qtms)
    if len(htms)>0:
