...
def getdict2(dict2step):
    global dict2
    predictstate=[(cc,ce,0,-1,-1)]
    newpredictstate=[]
    key2=dict21[tuple(cc)]*967680+dict22[tuple(ce)]
    dict2[key2]=1
    t0=time.time()
    for step in range(1,dict2step+1):
        t1=time.time()
        for cube in predictstate:
            oc,oe,oldstep,f1,f2=cube
            for r in phase2rotations:
                f,t=r//3,r%3+1
                if step==1 or f1!=f and not ((f1==5 and f==0 or f1==4 and f==2 or f1==3 and f==1) or (step>2 and f==f2 and (f==0 and f1==5 or f==1 and f1==3 or f==2 and f1==4))):
                    nc=oc.copy()
                    ne=oe.copy()
                    rotatecubeblock(f,t,nc,ne)
                    key2=dict21[tuple(nc)]*967680+dict22[tuple(ne)]
                    if key2 not in dict2:
                        newstep=oldstep+(3*f+3-t)*eighteen[step-1]
                        dict2[key2]=newstep+eighteen[step]
                        if step!=dict2step:
                            newpredictstate.append((nc,ne,newstep,f,f1))
                ...
                for t in range(1,4):
                    ...
                    if key1 in dict1:
                        ...
                        if m1_2!=1:
                            ...
                            #phase 2
                            key2=dict21[tuple(nc1)]*967680+dict22[tuple(ne1)]
                            if key2 in dict2:
                                m2=dict2[key2]
...
def rotatecubeblock(f,t,c,e):
    ...
    e1,e2,e3,e4=faceedge[f]
    ne1,ne2,ne3,ne4=facetimeedge[f][t]
    e[e1],e[e2],e[e3],e[e4]=e[ne1],e[ne2],e[ne3],e[ne4]
