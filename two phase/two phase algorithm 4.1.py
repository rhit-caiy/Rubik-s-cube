...
def phase1(c,cd,e,ed,phase1solutionnum,threadid):
    ...
    if key in dict1:
        ...
        if furtherstep=="":
            #phase 2
            solution=phase2([c,cd,e,ed],"",minstep)
            minstr=solution
            minstep=steplen(solution)
        else:
            solution1=phase2([c,cd,e,ed],furtherstep+"1",minstep)
            solution2=phase2([c,cd,e,ed],furtherstep+"3",minstep)
                ...
                if step==1 or str(f)!=previousstep[-2] and not (step>2 and str(f)==previousstep[-4] and previousstep[-4]+previousstep[-2] in ["05","50","13","31","24","42"]):
                    for t in range(1,4):
                        ...
                        if key in dict1:
                            ...
                            solution1=phase2(rotatecube(int(furtherstep[-1]),1,*phase2cube),newstep+furtherstep+"1",minstep)
                            if solution1!="6" and steplen(solution1)<minstep:
                                minstep=steplen(solution1)
                                minstr=solution1
                                print(threadid,minstep)
                            solution2=phase2(rotatecube(int(furtherstep[-1]),3,*phase2cube),newstep+furtherstep+"3",minstep)
...
def phase2(cubepack,s,currentminstep):
    c=cubepack[0]
    cd=cubepack[1]
    e=cubepack[2]
    ed=cubepack[3]
    ...
    maxstep=currentminstep-steplen(s)-dict2step-1
    if maxstep<1:
        return "6"
    cubes=[[c,cd,e,ed,s]]
    newcubes=[]
...
if __name__=="__main__":
    ...
    for i in range(cubenumber):
        ...
        a=executor.submit(phase1,*unsolvedcubes[0],phase1solutionnum,0)
        b=executor.submit(phase1,*unsolvedcubes[1],phase1solutionnum,1)
        c=executor.submit(phase1,*unsolvedcubes[2],phase1solutionnum,2)
        ar=a.result()
        print(ar)
        br=b.result()
        print(br)
